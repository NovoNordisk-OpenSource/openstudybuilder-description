...
Option | Description | Where to start
--|--|--
Local Desktop | The OpenStudyBuilder can be run on a local desktop environment | [Installation instructions](https://github.com/NovoNordisk-OpenSource/openstudybuilder-solution/blob/main/README.md){target=_blank}
Server Installation | OpenStudyBuilder can be set up to run on a server | [Accelerators repository](https://github.com/NovoNordisk-OpenSource/openstudybuilder-accelerators){target=_blank}
Cloud | The OpenStudyBuilder can be deployed on all major cloud platforms | [Accelerators repository](https://github.com/NovoNordisk-OpenSource/openstudybuilder-accelerators){target=_blank}
Hosted Cloud Environment | The OpenStudyBuilder can be hosted by a vendor | [Services](./info_services.md)
Sandbox | A public sandbox environment is available for evaluation purposes | [Request access](mailto:openstudybuilder@htp42.com?subject=Request%20Sandbox%20Access)
...
The current implementation requires **CDISC library access** via a library API key to access CDISC standards (freely available also for non-CDISC-members). This is used to receive CDISC standards and processed in the MDR-Standards-Import scripts which load for example new CDISC Controlled Terminology into the OpenStudyBuilder.

As OpenStudyBuilder is using a Neo4j graph database, a **Neo4j database license** is required. We highly recommend using the enterprise or AuraDB license which comes along with services and features recommended. It is also possible to use the GPLv3 licensed community edition which is available at no cost. OpenStudyBuilder can also run on the community edition, but some features like consistency checks will not be available. The migration scripts we provide to update the database schema from an old to a new version will also not be running and would need manual updating.
...
OpenStudyBuilder can run differently with Neo4j. It can use the Enterprise version or the AuraDB cloud database service. For AuraDB the following options are available:
...
To learn more, visit the [AuraDB website](https://neo4j.com/cloud/platform/aura-graph-database/){target=_blank}.

For more information about Neo4j licenses and deployment options:
